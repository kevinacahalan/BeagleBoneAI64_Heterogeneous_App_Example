...
void handle_request(MESSAGE *req_msg, RPMessage_Handle handle, uint32_t myEndPt, uint32_t remoteEndPt, uint32_t remoteProcId) {
    ...
    switch (req->function_tag) {
        ...
        case FUNCTION_B:
            ...
            break;
        default:
            printf("R5: Unknown request tag %d\n", req->function_tag);
            return;
    }
...
float call_linux_function_x_blocking(RPMessage_Handle handle, uint32_t myEndPt, uint32_t remoteEndPt, uint32_t remoteProcId, int param) {
    ...
    uint32_t start = TimerP_getTimeInUsecs();  // Use OSAL timer or similar
    while (TimerP_getTimeInUsecs() - start < 5000) {
        MESSAGE msg;
        uint16_t len = sizeof(MESSAGE);
        uint32_t srcEndPt = remoteEndPt;  // Temp vars
        ...
        if (status == IPC_SOK && len == sizeof(MESSAGE)) {
            if (msg.tag == MESSAGE_RESPONSE && msg.request_id == req_id) {
                if (msg.data.response.function_tag == FUNCTION_X) {
                    printf("R5: Got response for FUNCTION_X: %f\n", msg.data.response.result.result_function_x);
...
void send_command_x_nonblocking(RPMessage_Handle handle, uint32_t myEndPt, uint32_t remoteEndPt, uint32_t remoteProcId, int param) {
    MESSAGE cmd = {0};
    cmd.tag = MESSAGE_COMMAND;
    cmd.data.request.function_tag = FUNCTION_X;
    cmd.data.request.params.function_x.param = param;

    RPMessage_send(handle, remoteProcId, remoteEndPt, myEndPt, &cmd, sizeof(MESSAGE));
